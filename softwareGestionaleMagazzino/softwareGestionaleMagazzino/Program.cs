...
namespace prototipo_magazzino
{
    class Program
    {
        ...
        static string impostazioni = @"Impostazioni.txt";
            ...
            while (controllo == "")
            {
                switch (rispostaQuantitàFisse)
                {
                    case "0":
                        ...
                        StreamWriter riscritturaImpostazioni = new StreamWriter(percorsoFileImpostazioni, false);
        ...
        {
            if(!Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "fileGestione/"))
            {
                Console.WriteLine("Creazione cartelle essenziali in corso...");
                Directory.CreateDirectory(AppDomain.CurrentDomain.BaseDirectory + "fileGestione/");
                Console.WriteLine("Creata cartella principale.");
                if (!Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "fileGestione/fileGenerali/"))
                {
                    Directory.CreateDirectory(AppDomain.CurrentDomain.BaseDirectory + "fileGestione/fileGenerali/");
                    Console.WriteLine("Creata cartella dei file generali.");
                }

                if(!Directory.Exists(AppDomain.CurrentDomain.BaseDirectory + "fileGestione/fileProfitti/"))
                {
                    Directory.CreateDirectory(AppDomain.CurrentDomain.BaseDirectory + "fileGestione/fileProfitti/");
            ...
            {
                using (StreamWriter sw = File.CreateText(percorsoFileImpostazioni))
                {
